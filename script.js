...
var majMot, valider, mot, construire;
...
function bonmauvais() {
    ...
    tdd.bon = [];
    if (tdd.langue & 1) {
        tdd.bon = tdd.bon.concat(['Entier', 'Réel', 'Booléen', 'Chaîne', 'Tableau', 'Fonction', 'Objet']);
    }
    if (tdd.langue & 2) {
        tdd.bon = tdd.bon.concat(['Integer', 'Float', 'Boolean', 'String', 'Array', 'Function', 'Object']);
    }
    tdd.mauvais = [];
    if (tdd.langue & 1) {
        tdd.mauvais = tdd.mauvais.concat(['Date', 'Code Postal', 'Nom', 'Fichier', 'Variable', 'Alphabet', 'Personne', 'Base', 'Concaténation', 'Boucle', 'Mot de passe', 'Alternative', 'Propriété', 'Instruction']);
    }
    if (tdd.langue & 2) {
